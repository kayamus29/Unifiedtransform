                        ...
                        <div class="mb-4">
                                ...
                                <div class="col-sm-4 col-md-3">
                                    <div class="card bg-light">
                                        ...
                                        <div class="card-footer bg-white border-top-0 pt-0 pb-3 hstack gap-2">
                                            @if($student->status == 'active')
                                                <button type="button" class="btn btn-sm btn-outline-danger w-100" data-bs-toggle="modal" data-bs-target="#deactivateModal">
                                                    <i class="bi bi-person-x-fill me-1"></i> Deactivate
                                                </button>
                                            @elseif($student->status == 'deactivated')
                                                <form action="{{ route('student.reactivate', $student->id) }}" method="POST" class="w-100">
                                                    @csrf
                                    ...
                                    <div class="alert alert-warning border-0 shadow-sm mt-3 mb-0">
                                        ...
                                        <div class="smallest text-muted">By: {{ $student->deactivator->first_name ?? 'System' }} on {{ $student->deactivated_at->format('M d, Y') }}</div>
                                    </div>
                                    @endif
                                </div>
                                        ...
                                        <table class="table table-responsive mt-3">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{{ optional($student->parent_info)->father_name ?? 'N/A' }}</td>
                                                    <th>Mother's Name:</th>
                                                    <td>{{ optional($student->parent_info)->mother_name ?? 'N/A' }}</td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">Father's Phone:</th>
                                                    <td>{{ optional($student->parent_info)->father_phone ?? 'N/A' }}</td>
                                                    <th>Mother's Phone:</th>
                                                    <td>{{ optional($student->parent_info)->mother_phone ?? 'N/A' }}</td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">Address:</th>
                                                    <td colspan="3">
                                        ...
                                        <table class="table table-responsive mt-3">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <th>Board Reg. No.:</th>
                                                    <td>{{ optional($student->academic_info)->board_reg_no ?? 'N/A' }}</td>
                                                </tr>
                                                <tr>
                            ...
                            <div class="card bg-light mt-3">
                                <div class="card-body">
                                    ...
                                    <p class="card-text {{ $color }} small text-uppercase fw-bold">{{ $status }}</p>
                ...
                <form action="{{ route('student.deactivate', $student->id) }}" method="POST">
                    ...
                    <div class="modal-body py-4">
                        <div class="alert alert-danger border-0 small mb-4">
                            ...
                            This student will be blocked from logging in and excluded from future promotions and billing.
                        </div>
                        <div class="mb-3">
                            <label for="reason" class="form-label fw-bold small">Reason for Deactivation<sup><i class="bi bi-asterisk text-danger"></i></sup></label>
                            <textarea class="form-control" name="reason" id="reason" rows="3" placeholder="e.g. Disciplinary action, Voluntary withdrawal, etc." required></textarea>
                        </div>
                    </div>
